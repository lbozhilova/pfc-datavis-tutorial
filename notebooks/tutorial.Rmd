...
The "gg" in `ggplot2` stands for "grammar of graphics". If the grammar of a language is a way of describing the components of a sentence (subject, verb, object) and how they fit together, then a grammar of graphics is a way of describing the components of a plot (data, axes, shapes). Just like grammar differs from language to language, so it can differ from one data visualisation tool to another. However, when people talk about *the* grammar of graphics, they usually mean the grammar first described by [Leland Wilkinson](https://link.springer.com/book/10.1007/0-387-28695-0) and later extended and implemented by [Hadley Wickham](https://vita.had.co.nz/papers/layered-grammar.html).
...
Before we move on to code, let's illustrate the grammar of graphics. The figure below shows my running progress so far this year.
![](runs_example.jpg){width=50%}

One of my resolutions for 2022 is to do a certain amount of running over the course of the year. The **data** you see plotted above comes from the spreadsheet I use to keep track of my progress. There are a number of variables I record, but the only ones I have used for the plot are date, weekday, and distance.

The **aesthetic mappings** of the plot map the different data variables to things like axes, colours, shapes and sizes. In the plot above, there are three aesthetic mappings: dates are mapped to the x-axis, weekdays are mapped to colours (purple and orange), and distance is mapped to the y-axis.

Finally, **geometric objects** describe the types of plot you see. They can be things like histograms, box plots, heat maps, etc. One of the features which make `ggplot2` so versatile is that it allows us to layer different geometric objects  on top of each other to make one complex figure. To keep things simple, in the example above I used only one geometric object: a scatter plot.

Now that you have an idea of what data, aesthetic mappings, and geometric objects are, why don't you give it a try? Can you identify the different "blocks" of the plot below?

![](runs_exercise.jpg){width=50%}

## Materials and set-up

You should have `R` and RStudio installed. If not, first go to [CRAN](https://cran.r-project.org/) to download and install R, and afterwards head to the RStudio [website](https://www.rstudio.com/).
...
Finally, we need to load the packages.
```{r packages}
library("tidyverse")
library("cowplot")
```
