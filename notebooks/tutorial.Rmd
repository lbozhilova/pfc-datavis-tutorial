...
**Exercise 8: Positions.** Going back to our iris dataset, can you make the figure below? How do the column heights differ from earlier histograms we’ve made? As an additional exercise, can you see a difference between using `geom_line(stat = "bin")` and `geom_freqpoly()`? Hint: try adding colour to your plots.
...
In the plot above, I saved the first grid I showed you as `p`, and put it inside another grid with the scatter plot `q`. I also increased the relative height of the scatter plot to be twice that of the boxplots.
